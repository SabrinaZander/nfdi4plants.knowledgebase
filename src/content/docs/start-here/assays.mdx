...
<Card icon="pen" title="ARCitect">
    ...
    3. In the `New Table!` widget, click <Icons.Templates /> to `Start with template!`.
    4. Now all DataPLANT curated templates are shown. 
    5. To select a template from a different community click <Icons.Settings />. 
    6. Select "Training" community.
    7. Search for `Training - Sugar extraction`, select it and `Import Template`
    8. Select Import Type with `Units` and `Append to active table`. 
    9. You can click `Preview Selected Columns` <Icons.Settings /> to view all building blocks of the template. 
